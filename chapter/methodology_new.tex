...
The OEDB register containing information about run-of-the-river power plants is extracted from the Open Power Data Set and stored in Tab. ``ego{\_}dp{\_}res{\_}powerplant'' in the schema ``supply''. It is a register for renewable power plants, classified by technology through the column generation{\_}type (solar, wind, geothermal, run{\_}of{\_}river, gas, biomass). The map of OPDS run-of-the-river power plants and German rivers shows that the localization of the plants is not always accurate (see Fig. \ref{pp_river_dist}).
\begin{figure}[H]
\center
