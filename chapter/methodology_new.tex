...
A simulation will be run with daily measured data from the Global Runoff Data Center (GRDC, Koblenz, Germany) and the Federal Waterways and Shipping Administration, provided by the Bundesanstalt für Gewässerkunde. Table \ref{quelle_runoff_th} lists the gauge stations and the timespans over which data was made available. The stations of Zoellnitz and Kaulsdorf-Eichicht were not used in this study because they do not have available data over the last decade. Another simulation will be run with modeled runoff from the WaterGAP software. The runoffs are given per raster cell, as shown on Fig. \ref{raster_th} and available from 2006 to 2009, with daily values.
\begin{table}[H]
\footnotesize
 \centering
